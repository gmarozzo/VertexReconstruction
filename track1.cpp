...
double track :: Delta (double x0, double y0,double R){
...
bool track :: intpoint(double x0, double y0, double z0, double &X, double &Y, double &Z, double R){//valutazione punto di intersezione
  ...
  X=x0+c1*this->parameter(x0,  y0,this->Delta(x0,y0,R));
  Y=y0+c2*this->parameter(x0,  y0,this->Delta(x0,y0,R));
  Z=z0+c3*this->parameter( x0,  y0,this->Delta(x0,y0,R));
  if(abs(Z)<H/2){return true;}
  else{return false;}
}
